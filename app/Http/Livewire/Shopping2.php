...
class Shopping2 extends Component
{
    ...
    public function mount($shopcatId)
    {
        $shopcatDetail = Shopcat::find($shopcatId);

        if($shopcatDetail) {
            $this->shopcat = $shopcatDetail;
        }
    }
    ...
    public function render()
    {
        ...
        if($this->search) {
            $shopproducts = Shopproduct::where('shopcat_id', $this->shopcat->id)->where('name', 'like', '%'.$this->search.'%')->paginate(8);
        }else {
            $shopproducts = Shopproduct::where('shopcat_id', $this->shopcat->id)->paginate(8);
        }
        ...
        return view('livewire.shopping1', [                      
            ...
            'title' => $this->shopcat->image,
            'title1' => $this->shopcat->name,
            'title2' => $this->shopcat->description,
            'title3' => 'Category '.$this->shopcat->name
        ]);
    }
}
